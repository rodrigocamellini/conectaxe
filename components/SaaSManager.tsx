      ...
      <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        <div className="bg-white rounded-[3rem] shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="px-10 pb-10 grid grid-cols-1 md:grid-cols-2 gap-4">
             <button onClick={() => setShowPayModal(true)} className="p-6 bg-emerald-50 border border-emerald-100 rounded-[2rem] flex items-center gap-5 hover:scale-[1.02] transition-all text-left"><div className="p-4 bg-emerald-500 text-white rounded-2xl shadow-lg shadow-emerald-500/20"><QrCode size={24} /></div><div><p className="text-sm font-black text-emerald-800 uppercase leading-none mb-1">Regularizar Plano</p><p className="text-xs text-emerald-600 font-medium">Ver dados de pagamento (PIX / Banco)</p></div></button>
        ...
        {showPayModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[110] p-4">
            <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in duration-300">
               ...
               <div className="p-8 space-y-8"><div className="space-y-4"><div className="flex items-center justify-between"><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Chave PIX</h4><button onClick={() => copyToClipboard(masterData.pixKey)} className="text-[9px] font-black text-emerald-600 uppercase flex items-center gap-1 hover:underline"><Copy size={10} /> Copiar Chave</button></div><div className="p-4 bg-gray-50 border-2 border-emerald-100 rounded-2xl flex items-center gap-4"><div className="p-3 bg-pink-100 rounded-xl flex items-center justify-center overflow-hidden shrink-0"><QrCode size={32} className="text-pink-600" /></div><p className="font-black text-slate-700 break-all">{masterData.pixKey}</p></div></div><div className="space-y-4">
                 <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Depósito / Transferência</h4>
                 <div className="p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl flex items-center gap-4">
                    <div className="p-3 bg-indigo-100 rounded-xl flex items-center justify-center overflow-hidden shrink-0">
                       <Landmark size={32} className="text-indigo-600" />
                    </div>
                    <pre className="font-sans text-sm font-bold text-slate-600 whitespace-pre-wrap">{masterData.bankDetails}</pre>
                 </div>
              </div><div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100"><p className="text-xs text-indigo-700 font-medium text-center">Após efetuar o pagamento, envie o comprovante informando seu ID <span className="text-red-600 font-bold">{formattedClientId}</span>.</p></div><button onClick={() => setShowPayModal(false)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest">Entendi, fechar</button></div>
            </div>
          </div>
        )}
      </div>
